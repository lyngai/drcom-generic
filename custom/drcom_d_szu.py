...
dhcp_server = '172.29.34.254'
AUTH_VERSION = '\x23\x00'
mac = 0x80fa5b1cc0b2
KEEP_ALIVE_VERSION = '\xdc\x02'
#config_end
host_os = 'DrCOM\x05\xcf\x07\x04\x00\x63\x6b\x20\x31' #DrCOM
...
def version():
	print "=====================================================================" 
	print "DrCOM Auth Router for SZU" 
...
def mkpkt(salt, usr, pwd, mac):
    ...
    data += '\xc0\xa8\x5c\x01' #your ipaddress 2
    data += '\xc0\xa8\xaf\x01' #your ipaddress 3
    data += '\00'*4 #your ipaddress 4
    data += md5sum(data + '\x14\x00\x07\x0b')[:8] #md53
    data += IPDOG
    data += '\x00'*4 #delimeter
    data += host_name.ljust(32, '\x00')
    data += '\xca\x60\x80\xa6' #primary dns: 114.114.114.114
    data += '\xac\x1d\x22\xfe' #DHCP server
    data += '\xca\x60\x86\x85' #secondary dns:8.8.8.8
    data += '\x00' * 8 #delimeter
    data += '\x94\x00\x00\x00' # unknow
    data += '\x06\x00\x00\x00' #os major
    data += '\x01\x00\x00\x00' # os minor
    data += '\xb1\x1d\x00\x00' # OS build
    data += '\x02\x00\x00\x00' #os unknown
    data += host_os.ljust(32,'\x00')
    data += '\x00' * 32
    #unknown data start
    data += '\x38\x63\x39\x37\x63\x66\x61\x64'
    data += '\x30\x32\x31\x33\x38\x65\x31\x34'
    data += '\x31\x38\x65\x32\x39\x31\x38\x64'
    data += '\x61\x34\x39\x65\x66\x64\x61\x35'
    data += '\x35\x32\x35\x30\x34\x30\x36\x62'.ljust(32,'\x00')
    #unknown data end
